...
//   String otpGotten = "";
//   bool isResendOtpEnabled = true;
//   int resendOtpSeconds = 60;
//   Timer? resendOtpTimer;
//   bool isContinueButtonEnabled = false; // Add this variable
...
//         pinCodeController.text.isNotEmpty; // Update button state
//     notifyListeners();
//   }
...
class ChangePinViewModel extends BaseViewModel {
  User user = User();
  GetOtpResponse? previousResponse;
  String? newPin; // Store the new pin here
  ...
  onChange(String? val) {
    isContinueButtonEnabled = pinCodeController.text.isNotEmpty;
    notifyListeners();
  }
  ...
  submit() async {
    ...
    try {
      ...
      if (response?.status == true) {
        newPin = newPinController.text.trim(); // Store the new pin
        showCustomToast("Pin Changed Successfully", success: true);
        stopLoader();
        notifyListeners();
        await showSuccesses();
  ...
  verifyOTP() async {
    ...
    try {
      ...
      if (response?.status == true) {
        var userData = UserData(otp: pinCodeController.text.trim(), password: pinID);
        appCache.userData = userData;
        stopLoader();
        showCustomToast("Otp Verified Successfully", success: true);
        navigationService.navigateTo(changePinRoute);
  ...
  sendOtp() async {
    ...
    try {
      ...
      if (response?.status == true) {
        ...
        appCache.forgetPasswordResponse = response ?? GetOtpResponse();
        previousResponse = response;
        showCustomToast("OTP sent to ${userService.user.phoneNumber}", success: true);
      }
    } catch (err) {}
  }
